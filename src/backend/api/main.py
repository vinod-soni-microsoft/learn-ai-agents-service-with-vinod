...
def create_app():
    ...
    app.add_middleware(
        CORSMiddleware,
        allow_origins=[frontend_url, "http://localhost:3000", "http://localhost:5173"],  # Add frontend URLs
        allow_credentials=True,
        allow_methods=["*"],
        allow_headers=["*"],
    )
    
    # Static files are not needed in separated architecture
    # app.mount("/static", StaticFiles(directory=directory), name="static")
    
    # Mount React static files
    # Removed - frontend is now served separately
